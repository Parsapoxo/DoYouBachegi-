...
const APP_VERSION = "1.0.0"; // keep in sync with CHANGELOG.md — bump on every change
...
// in the "Others" list. Poster files are expected at posters/<poster>.png
// --------------------------------------------------------------------------
...
// Single-installment movies -> "Others" section
const OTHERS = [
...
function posterCard(movie) {
  ...
  card.innerHTML = `
    <div class="poster-frame">
      <img class="poster-img" src="posters/${movie.poster}.png" alt="${movie.title}" loading="lazy" />
...
function renderMovies() {
  ...
  othersSection.className = "studio-section";
  const h2 = document.createElement("h2");
  h2.className = "studio-title";
  h2.textContent = "Others";
  othersSection.appendChild(h2);
  const grid = document.createElement("div");
  grid.className = "poster-grid";
  OTHERS.forEach((m) => grid.appendChild(posterCard(m)));
  othersSection.appendChild(grid);
  container.appendChild(othersSection);

  // paint watched state on everything
  [...STUDIOS.flatMap((s) => s.franchises.flatMap((f) => f.movies)), ...OTHERS].forEach((m) => renderPosterState(m.id));
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  renderAvatarGrid(document.getElementById("signup-avatar-grid"), true, (id) => (signupAvatar = id), signupAvatar);
